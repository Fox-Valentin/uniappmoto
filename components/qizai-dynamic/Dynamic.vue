		...
		<view class="user__container">
			...
			<view class="user__content">
				<view class="user__content-main">
					<text class="user__content-title uni-ellipsis" @click.stop="clickUser()">{{ item.nickName }}</text>
					<text class="user__content-note uni-ellipsis">{{ item.personSignature }}</text>
				</view>
				<view class="user__content-extra">
					<slot>
						<text :class="item.isFocus?'user__focus-off':'user__focus-on'"
							@click.stop="clickFocus()">{{item.isFocus?'已关注':'关注' }}</text>
					</slot>
				</view>
			</view>
		</view>
		<view class="text">{{item.feedContent}}</view>
		<view class="allImage" v-if="item.video">
			<view class="imgList">
				<video :style="{width:imgWidth+'px','max-height':imgHeight+'px'}" :src="item.video"></video>
			</view>
		</view>
			...
			<uni-grid :column="2" :showBorder="false" :square="false">
				<uni-grid-item>
					<span :style="'color:'+thumbsupColor" @click.stop="clickThumbsup()">
						<uni-icons type="hand-thumbsup-filled" size="18"
				...
				<uni-grid-item>
					<span style="color:gray" @click.stop="clickChat()">
						...
						{{item.feedCommentCount?item.feedCommentCount:'评论'}}
					</span>
				</uni-grid-item>
			</uni-grid>
		</view>
...
<script>
	export default {
		...
		mounted() {
			...
			this.imgList = JSON.stringify(this.item.images);
			this.judgeImg();
			//this.initOperate();
		},
		methods: {
			...
			// 自适应判断
			judgeImg() {
				if (this.item.video || this.imgList.length == 1) {
